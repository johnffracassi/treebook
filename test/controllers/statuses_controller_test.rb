...
class StatusesControllerTest < ActionController::TestCase
  ...
  test "should create status for current user when logged in" do
    ...
    assert_difference('Status.count') do
      post :create, status: { content: @status.content, user_id: users(:herb).id }
    end

    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:johnfrac).id
  end
  ...
  test "should update status for current user when logged in" do
    sign_in users(:johnfrac)
    put :update, id: @status, status: { content: @status.content, user_id: users(:herb).id }
    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:johnfrac).id
  end
  ...
  test "should not update status if nothing has changed" do
    ...
    assert_raises(ActionController::ParameterMissing) do
      ActionController::Parameters.new(:status => {}).require(:status)
    #assert_redirected_to status_path(assigns(:status))
        #assert_equal assigns(:status).user_id, users(:johnfrac).id
    end
  end
